...
def GetVoltageDataExcel():
    ...
    if os.path.exists(path):
        ...
        return send_file('/home/sanjay/Codes/MIS/Voltage.xlsx', as_attachment=True, download_name=custom)
    else:
...
def GetLinesDataExcel():
    ...
    if os.path.exists(path):
        ...
        response = Response(
            data, content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    return send_file('/home/sanjay/Codes/MIS/Line.xlsx', as_attachment=True, download_name=custom)
...
def GetDemandCollection():
    ...
    CONNECTION_STRING = "mongodb://forecast_user:forecast@10.3.101.90:27017/?authSource=admin"
    client = MongoClient(CONNECTION_STRING)
    db = client['load_forecast']
    actual_demand = db['actual_demand']
    forecast_demand = db['forecast_demand']
    return actual_demand, forecast_demand


actual_demand, forecast_demand = GetDemandCollection()
...
def DemandNames():
    ...
    cursor = actual_demand.find(
        filter=filter, projection={'n': 1}).distinct('n')
    cursor1 = forecast_demand.find(
        filter=filter, projection={'n': 1}).distinct('n')
...
def GetDemandData():
    ...
    for station in stationName:
        ...
        for it in date_range:
            ...
            actual_result = actual_demand.find(
                filter=filter,
                projection=project
            )

            actual_list = list(actual_result)

            if len(actual_list) == 0:
                actual = actual + listofzeros

            else:

                actual = actual + actual_list[0]['b']

            forecast_result = forecast_demand.find(
                filter=filter,
                projection=project
            )

            forecast_list = list(forecast_result)

            if len(forecast_list) == 0:
                forecast = forecast + listofzeros

            else:

                forecast = forecast + forecast_list[0]['b']
        ...
        actual = actual[allDateTime.index(
            startTime):allDateTime.index(endTime)+1]

        forecast = forecast[allDateTime.index(
            startTime):allDateTime.index(endTime)+1]

        df1 = pd.DataFrame.from_dict({station+': Actual': actual})

        merge_list.append(df1)

        df1 = pd.DataFrame.from_dict({station+': Forecast': forecast})
        ...
        if time1 == 15:
            ...
            x = list(divide_chunks(actual, time1//15))
            y = list(divide_chunks(forecast, time1//15))

            for item in x:
                max_1, min_1, avg_1 = my_max_min_function(item)
                temp_actual.append(avg_1)

            for item in y:
                max_2, min_2, avg_2 = my_max_min_function(item)
                temp_forecast.append(avg_2)

            data = {'stationName': station,
                    'actual': temp_actual, 'forecast': temp_forecast}
            reply.append(data)

    merged = pd.concat(merge_list, axis=1, join="inner")

    merged.to_excel("Demand.xlsx", index=None)

    for i in range(len(reply)):

        max_v1, min_v1, avg_v1 = my_max_min_function(reply[i]['actual'])
        max_v2, min_v2, avg_v2 = my_max_min_function(reply[i]['forecast'])

        if max_v1[0] == 0 and min_v1[0] == 0:
            max_v1 = ["Sorry data isn't available for this selection"]
            min_v1 = ["Sorry data isn't available for this selection"]

        elif len(max_v1) > 2 and len(min_v1) > 2:
            max_v1 = [max_v1[0], "Same Data at every point"]
            min_v1 = [min_v1[0], "Same Data at every point"]

        else:
            for x in range(1, len(max_v1)):
                max_v1[x] = allDateTime[max_v1[x]]

            for y in range(1, len(min_v1)):
                min_v1[y] = allDateTime[min_v1[y]]

        if max_v2[0] == 0 and min_v2[0] == 0:
            max_v2 = ["Sorry data isn't available for this selection"]
            min_v2 = ["Sorry data isn't available for this selection"]

        elif len(max_v2) > 2 and len(min_v2) > 2:
            max_v2 = [max_v2[0], "Same Data at every point"]
            min_v2 = [min_v2[0], "Same Data at every point"]

        else:
            for x in range(1, len(max_v2)):
                max_v2[x] = allDateTime[max_v2[x]]

            for y in range(1, len(min_v2)):
                min_v2[y] = allDateTime[min_v2[y]]
        ...
        for station in stationName:
            ...
            for dateval in MultistartDate:
                ...
                if time1 == 15:
                    ...
                    data = {'stationName': station, 'actual': actual,
                            'forecast': forecast, 'Date_Time': DateObj}
                    reply.append(data)
                    ...
                    data = {'stationName': station, 'actual': temp_actual,
                            'forecast': temp_forecast, 'Date_Time': DateObj}
                    reply.append(data)
        ...
        for station in stationName:
            ...
            for dateval in MultistartDate:
                ...
                if time1 == 15:
                    ...
                    data = {'stationName': station, 'actual': actual,
                            'forecast': forecast, 'Date_Time': startDateObj}
                    reply.append(data)
...
def GetDemandDataExcel():
    ...
    if os.path.exists(path):
        ...
        response = Response(
            data, content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    return send_file('/home/sanjay/Codes/MIS/Demand.xlsx', as_attachment=True, download_name=custom)


def InsertDemandDfIntoDBtest(line_mw_data_collection, line_mw_data_collection1, df, for_date):
...
@app.route('/DemandUpload', methods=['GET', 'POST'])
def DemandUpload():
    file = request.files["demo[]"]
    ...
    if file:
        ...
        for item in range(len(df)//1440):
            ...
            if (len(fd) == 1440):
                ...
                InsertDemandDfIntoDBtest(
                    actual_demand, forecast_demand, fd, for_date)
...
@app.route('/MVARNames', methods=['GET', 'POST'])
def MVARNames():
...
@app.route('/MultiMVARNames', methods=['GET', 'POST'])
def MultiMVARNames():
...
@app.route('/GetMVARData', methods=['GET', 'POST'])
def GetMVARData():
    ...
    merged.to_excel("MVAR.xlsx", index=None)
...
@app.route('/GetMultiMVARData', methods=['GET', 'POST'])
def GetMultiMVARData():
...
@app.route('/GetMVARDataExcel', methods=['GET', 'POST'])
def GetMVARDataExcel():
    ...
    if startDate == endDate:
        custom = startDate+' '+MVAR_file_name+' MVAR Data of '+names+'.xlsx'

    else:
        custom = startDate+' to '+endDate+' ' + \
            MVAR_file_name+' MVAR Data of '+names+'.xlsx'
    ...
    return send_file('/home/sanjay/Codes/MIS/MVAR.xlsx', as_attachment=True, download_name=custom)


def InsertMVARDfIntoDBtest(line_mw_data_collection, line_mw_data_collection1, df, for_date):
...
@app.route('/MVARUpload', methods=['GET', 'POST'])
def MVARUpload():
    file = request.files["demo[]"]
    ...
    if file:
        ...
        for item in range(len(df)//1440):
            ...
            if (len(fd) == 1440):
                ...
                InsertMVARDfIntoDBtest(MVAR_P1, MVAR_P2, fd, for_date)
...
def ICTFileInsert():
    ...
    def insertFlowDfIntoDB(ICT_data, df, for_date):
        ...
        try:
            ...
            print("Successfully inserted ICT Files", for_date)

        except:
            print("ICT File Insert problem in Database ")
    ...
    for for_date1 in pd.date_range(date(startDateObj.year, startDateObj.month, startDateObj.day), date(endDateObj.year, endDateObj.month, endDateObj.day)):
        ...
        try:
            ...
            for col in df.columns:
                ...
                insertFlowDfIntoDB(ICT_data1, sub_df, for_date1)
...
def ICTFileInsertMW(startDate, endDate):
    ...
    def insertFlowDfIntoDB(ICT_data, df, for_date):
        ...
        try:
            ...
            res = ICT_data.insert_one(a)
            print('ICT File Inserted successfully', for_date)
    ...
    for for_date1 in pd.date_range(date(startDateObj.year, startDateObj.month, startDateObj.day), date(endDateObj.year, endDateObj.month, endDateObj.day)):
        ...
        try:
            ...
            for col in df.columns:
                ...
                insertFlowDfIntoDB(ICT_data2, sub_df, for_date1)
...
def GetICTDataExcel():
    ...
    return send_file('/home/sanjay/Codes/MIS/ICT.xlsx', as_attachment=True, download_name=custom)
...
def GetFrequencyDataExcel():
    ...
    if os.path.exists(path):
        ...
        response = Response(
            data, content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    return send_file('/home/sanjay/Codes/MIS/Frequency.xlsx', as_attachment=True, download_name=custom)
...
def GetDemandMinDataExcel():
    ...
    if os.path.exists(path):
        ...
        response = Response(
            data, content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    return send_file('/home/sanjay/Codes/MIS/DemandMin.xlsx', as_attachment=True, download_name=custom)
...
def GetGeneratorDataExcel():
    ...
    if os.path.exists(path):
        ...
        response = Response(
            data, content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    return send_file('/home/sanjay/Codes/MIS/Generator.xlsx', as_attachment=True, download_name=custom)
...
def GetISGSDataExcel():
    ...
    if os.path.exists(path):
        ...
        response = Response(
            data, content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    return send_file('/home/sanjay/Codes/MIS/ISGS.xlsx', as_attachment=True, download_name=custom)
